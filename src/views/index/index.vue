...
<div class="index" :style="{ height: pageHeight + 'px' }">
  <div class="content">
      ...
      <div class="bodyItem bodyLeft">
        ...
        <div class="alarmStatistics">
          ...
          <ul class="taskList flex_start_v">
            <li class="flex_start_v" :style="{ backgroundImage: `url(${item.icon})` }" v-for="item in alarmMap" :key="item.id">
              <div>
      ...
      <div class="bodyItem bodyCenter">
        <div class="count-container">
          <div class="count-header">
            ...
            <span class="count">
              <div class="number-item" :key="index" v-for="(item, index) in String(securityCheckTotal).split('')">
                {{ item }}
              </div>
            </span>
          </div>
          ...
          <div class="device-content">
            ...
            <ul class="camera-tab">
              <li :class="
                    videoTypeTab.activeVideoTab === item.tabKey
                      ? 'active button'
                      : 'button'
                  " :key="item.tabKey" v-for="item in videoTypeTab.subTabs" @click="(e) => setActiveTab(item.tabKey)">
                {{ item.name }}
              </li>
            </ul>
            <div class="video-container">
              ...
              <div class="video-list-container">
                <ul class="video-list">
                  <li @click="(e) => setActiveAlarm(item, e)" :key="item.time" v-for="item in videoList" class="video-item">
                    <span class="time">{{ item.gmtCreate }}</span>
                    <span :style="{ color: colorMap[item.alarmLevel] }" class="status">{{ item.statusDesc }}</span>
                    <img v-show="activeRecordId && activeRecordId.id === item.id" class="activeTag" src="../../assets/images/alarmModalActiveTag.png" alt />
                  </li>
                </ul>
              </div>
            </div>
    ...
    <el-dialog title="历史记录详情" :append-to-body="true" width="90%" @close="activeRecordId = null" :visible="!!activeRecordId">
      <detail :id="activeRecordId"></detail>
    </el-dialog>
  </div>
